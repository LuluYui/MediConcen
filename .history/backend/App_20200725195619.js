...
let initDatabase = `CREATE TABLE if not exists users(
  name VARCHAR(50), id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, 
  password INT NOT NULL, email VARCHAR(50) NOT NULL UNIQUE,
  clinicname VARCHAR(50), phonenum INT,
  address VARCHAR(255), created_at DATETIME DEFAULT CURRENT_TIMESTAMP);`

let secInit = `CREATE TABLE IF NOT EXISTS consultation_record(id INT PRIMARY KEY AUTO_INCREMENT,
...
connection.connect(function (err) {
  ...
  connection.changeUser({database : 'Mediconcen'}, function(err) {
    if (err) throw err;
  });

  connection.query(initDatabase, (error, results, fields) => {
  ...
  connection.query(secInit, (error, results, fields) => {
    console.log("Initialize database Mediconcen")
    if (error) {
      return console.error(error.message);
    }
