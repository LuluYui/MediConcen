...
app.post('/registration', function(req, res){
  ...
  connection.query(
    ...
    "SELECT * FROM users WHERE email = ? AND password = ?",
    [email, password], function(err, exitsResults, field) {
      if (err) {
        console.log(err);
        res.send({ 'success': false, 'message': 'Could not connect database'});
      }
      ...
      if (results.length > 0 ){
        res.send({success: false, message: 'User '+ exitsResults[0].name+ ' exists'});
        ...
        connection.query(
          ...
          function(err, results, field) {
            ...
            if (results.length > 0 ){
              ...
              res.send({success: true, message: exitsResults[0].name + ' is added'});
