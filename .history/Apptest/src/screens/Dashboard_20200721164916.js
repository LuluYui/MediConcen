...
export default class AgendaScreen extends Component {
  ...
  renderItem(item) {
    return (
      ...
      >
        <Text>{ this.timeToStringClock(item.date)} </Text>
        <Text>ID: {item.id}</Text>
        <Text>Doctor: {item.doctorname} </br> Patient: {item.patientname} </Text>
        <Text>Diagnosis: {item.diagnosis}  Medication: {item.medication}</Text>
        <Text>Fee: ${item.consultation_fee}</Text>
        <Text>Followup: {item.followup}   Remarks: {item.remarks}</Text>
  ...
  timeToStringClock(time) {
    ...
    return 'Time: ' + result + ' - ' + endHour + ':' + endTime;
